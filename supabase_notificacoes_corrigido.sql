-- ============================================================
-- AeroMoc Aviation — Tabela de Notificações (CORRIGIDO)
...
-- 4. Políticas RLS - VERSÃO CORRIGIDA
-- Qualquer autenticado pode ler notificações ativas
DROP POLICY IF EXISTS "Notificacoes SELECT" ON public.notificacoes;
CREATE POLICY "Notificacoes SELECT" ON public.notificacoes
  FOR SELECT
  TO authenticated
  USING (ativo = true);
...
CREATE POLICY "Notificacoes INSERT" ON public.notificacoes
  FOR INSERT
  TO authenticated
...
CREATE POLICY "Notificacoes UPDATE" ON public.notificacoes
  FOR UPDATE
  TO authenticated
...
CREATE POLICY "Notificacoes DELETE" ON public.notificacoes
  FOR DELETE
  TO authenticated
  USING (auth.uid() = instrutor_id);
